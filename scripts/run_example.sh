...
usage() {
  cat <<'EOF'
Run the AppTrace example app with Supabase credentials.

Defaults to Chrome (web). Pass `-d <device>` after `--` to use another target.

Usage:
  ./scripts/run_example.sh [options] [-- flutter-run-args]

Options:
  --url <url>     Supabase project URL
  --key <key>     Supabase anon/publishable key
  --env <file>    Load variables from a .env file (default: example/.env)
